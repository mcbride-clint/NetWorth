...
**NetWorth** is a Blazor WebAssembly app (.NET 8) for tracking household net worth. All data is stored in browser localStorage — no backend/API.
...
- **[Services/StatementService.cs](NetWorth/Services/StatementService.cs)** — Singleton state manager. Maintains `Saved` (persisted) and `Current` (working copy) states. `SaveAsync()` commits to localStorage; `ResetAsync()` reverts.
...
- **[wwwroot/js/](NetWorth/wwwroot/js/)** — JS interop for localStorage (`statementStorage.js`) and file import/export (`statementFileInterop.js`).
...
JSON serialized to localStorage via JS interop. File export/import also supported for backup/restore.
